class Gerbil::CastsController < ApplicationController
  # GET /casts
  # GET /casts.json
  def index
    @casts = Cast.all
  ...
  def update
    ...
    respond_to do |format|
      if @cast.update_attributes(params[:cast])
        ...
        format.html { render action: "edit", notice: "Could not comply: #{@cast.errors}"  }
        format.json { render json: @cast.errors, status: :unprocessable_entity }
      end
    end
  end
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to gerbil_casts_url }
      format.json { head :no_content }
    end
  end
end
